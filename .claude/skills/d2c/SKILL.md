...
1. 基于实际项目布局的带指标 ASCII 结构图。主要容器直接标注关键尺寸或 `fixed`、`hug`、`fill/stretch`、`min/max` 行为，以及 padding、gap 和对齐。简单节点使用一张图；复杂节点先给页面或节点总览，再按具有独立布局逻辑的主要组件拆分子图，不按 Figma 图层机械拆分；总览与子图保持组件名称一致。
2. 补充 ASCII 图不宜承载的布局细节和响应式变化，并区分 Figma 明确值、布局计算结果和项目规则调整值。
3. 核心样式：字体、字号、字重、行高、字间距、颜色、圆角、边框、阴影和状态。
