...
1. 基于实际项目布局的 ASCII 结构图。完整页面显示顶层结构并按区块摘要；小组件使用紧凑结构。
2. 核心布局指标：容器、尺寸、间距、对齐和响应式行为；对相关容器标明 `fixed`、`hug`、`fill/stretch` 或 `min/max` 等尺寸行为，并区分 Figma 明确值、布局计算结果和项目规则调整值。
