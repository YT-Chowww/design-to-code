...
D2C 支持官方 Figma MCP 和 Figma-Context-MCP。用户未指定且两者都可用时，默认使用官方 Provider。
...
`D2C.md` 保存团队长期维护的技术栈、组件、Token、布局、资源、修改边界和验证规则。Skill 从目标项目向上查找到当前仓库根，采用适用的规则，同项以就近规则为准；无 Git 仓库时只检查目标目录。

首次执行时：

1. 未找到适用的 `D2C.md` 时，对当前项目做有界分析，在对话中展示候选内容；用户确认后才写入目标项目根目录。
