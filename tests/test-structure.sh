...
for skill in d2c d2c-extract d2c-generate d2c-validate d2c-verify d2c-merge; do
  ...
  assert "Skill 有 name 字段: $skill" "grep -q 'name:.*$skill' '$FILE'"
  assert "Skill 有 description 字段: $skill" "grep -q 'description:' '$FILE'"
done

echo ""
echo "=== Template Project ==="
assert "package.json 存在" "[ -f templates/vite-preview/package.json ]"
assert "package.json 是合法 JSON" "python3 -c \"import json; json.load(open('templates/vite-preview/package.json'))\""
assert "vite.config.ts 存在" "[ -f templates/vite-preview/vite.config.ts ]"
assert "tsconfig.json 存在" "[ -f templates/vite-preview/tsconfig.json ]"
assert "tsconfig.node.json 存在" "[ -f templates/vite-preview/tsconfig.node.json ]"
assert "index.html 存在" "[ -f templates/vite-preview/index.html ]"
assert "main.ts 存在" "[ -f templates/vite-preview/src/main.ts ]"
assert "App.vue 存在" "[ -f templates/vite-preview/src/App.vue ]"
assert "env.d.ts 存在" "[ -f templates/vite-preview/src/env.d.ts ]"
assert "App.vue 使用 script setup" "grep -q 'script setup' templates/vite-preview/src/App.vue"
...
assert "design-system.md 存在" "[ -f context/design-system.md ]"
assert "design-system.md 包含颜色定义" "grep -q 'color' context/design-system.md"
assert "design-system.md 包含字体定义" "grep -q 'font' context/design-system.md"
assert "design-system.md 包含间距定义" "grep -q 'spacing' context/design-system.md"
assert "component-library.md 存在" "[ -f context/component-library.md ]"
assert "project-config.md 存在" "[ -f context/project-config.md ]"
